...
def get_or_build_tokenizer(config, ds, lang):
    ...
    if not Path.exists(tokenizer_path):
        ...
        trainer = WordLevelTrainer(
            special_token=["[UNK]", "[PAD]", "[SOS]", "[EDS]"], min_frequency=2)
...
def get_ds(config):
    ...
    # Take 90% for training and 10% for validation
...
def train_model(config):
    ...
    for epoch in range(inital_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            encoder_ouput = model.encode(
                encoder_input, encoder_mask)  # (B, Seq_len, d_model)
            decoder_output = model.decode(
                encoder_ouput, encoder_mask, decoder_input, decoder_mask)  # (B, Seq_len, d_model)
